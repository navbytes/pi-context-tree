...
**v0.5 changes (gauge display mode, 2026-08-27):** `/gauge bar|border` (F5.6) — the gauge can render into pi's input border instead of a pinned widget. Accepted only at label parity with the bar: both modes share `gaugeLabel()`, so neither can drift.
...
- F5.6 Gauge display mode (`/gauge bar|border`, persisted in `~/.pi/agent/pi-context-tree.json`, default `bar`). `bar` pins the gauge widget above the editor; `border` paints it into the input box's bottom border, buying back a line of vertical space. **Parity is the requirement, not an aspiration:** both modes render the same `gaugeLabel()` — token counts, percent, band word, the `est` marker, the compaction notice — plus the trend. pi already draws text inside that border (`─── ↓ 3 more ────`), so a border-mode gauge that showed only a color would be discarding the information the gauge exists to deliver. Narrow terminals drop the label and keep the bar; pi's scroll indicator is preserved. TUI only — `setEditorComponent` is a no-op in RPC/headless, where the mode falls back to `bar` rather than showing nothing.
