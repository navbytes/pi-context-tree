...
| **`/gauge bar\|border`** | Where the health gauge lives — a bar pinned above the prompt, or painted into the input box's own border. Same reading either way. |
...
A **context-health gauge bar pinned above the prompt** (`CONTEXT ▓▓░ … N% band`, green→red, band ticks at 8k/32k/64k tokens, drawn as a share of your window). It carries a **`▲` trend** when context is filling fast and **attributes jumps** — `… 46k / 200k · 23.2% filling ▲ +24% (chrome.snapshot)` names *what* just bloated the window, right where you'll see it. The footer stays compact (`⎇ branch · ctx N% band`); the trend is rendered once, on the bar. While pi is still estimating (right after a session loads) it stays honest: the band word + a coarse `~est`, never a fake-precise percent. `/gauge border` moves the whole reading into the input box's bottom border — `─ 46k / 200k · 23.2% filling ───────·······` — which buys back a line without losing anything, since both modes render the same label. Plus a footer status `⎇ branch · ctx N% band`, a terminal title color-hashed per branch, a one-time nudge when context enters the red band (with hysteresis, so hovering the boundary does not re-fire it), a separate warning before pi's own auto-compaction is about to swap your source material for a summary, and a philosophy warning on `/compact`.
