/**
 * Decision-record drafting via the branch's own model (F2.2), through pi's
 * public model registry — pi owns auth (API keys, OAuth, provider config), so
 * this no longer fetches credentials and calls pi-ai's complete() itself.
 */

import type { DraftFn, ModelLike } from "./adapter.ts";
